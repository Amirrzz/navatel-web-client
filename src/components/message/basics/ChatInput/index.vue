    ...
    <div class="chat-input">
      ...
      <div class="chat-items-container" v-show="!showRecorder">
        ...
        <IonIcon
          ...
          alt="icon attach file"
          color="dark"
          icon="/Images/tabs/voice.svg"
          v-show="textMessage == ''"
        ></IonIcon>
      </div>
    </div>
    <EmojiPicker v-show="false" @update="addEmojiToTextMessage"></EmojiPicker>
  </div>
</template>
...
const enterSendMessageHandler = (event) => {
  ...
  if (
    themeStore.platforms.includes('mobileweb') ||
    themeStore.platforms.includes('mobile')
  )
    return;
