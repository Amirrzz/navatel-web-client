  ...
  <div class="contact-container" @click="addToGroup">
    ...
    <div class="content">
      <div class="user-information">
        ...
        <div class="time">
          <span v-if="item.lastSeen && item.lastSeen == 'yesterday'">{{
            $t('tabs.contacts.lastSeen', { time: t('tabs.contacts.yesterday') })
          }}</span>

          <span v-if="item.lastSeen && item.lastSeen != 'yesterday'">
            {{ $t('tabs.contacts.lastSeen', { time: item.lastSeen }) }}
          </span>
        </div>
      </div>
    </div>
...
const addToGroup = () => {
  const member = { blob: gettingImage.value, ...props.item };
  overallChatsStore.addMemberToGroup(member);
};

const memberInGroup = computed(() => {
  const membersID = overallChatsStore.groupMembersClient
